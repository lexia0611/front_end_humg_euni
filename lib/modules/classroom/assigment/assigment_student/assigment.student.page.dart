...
class AssigmentStudentPage extends StatefulWidget {
  const AssigmentStudentPage({super.key});

  @override
  State<AssigmentStudentPage> createState() => _AssigmentStudentPageState();
}

class _AssigmentStudentPageState extends State<AssigmentStudentPage> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AssigmentStudentCubit, AssigmentStudentCubitState>(
                        ...
                        child: DropdownButton2(
                          ...
                          onChanged: (value) {
                            setState(() {
                              ...
                              // if()
                              if (statusQuiz == -1) {
                                context.read<AssigmentStudentCubit>().getListAssigmentTeacher();
                              } else {
                                context.read<AssigmentStudentCubit>().getListAssigmentTeacher(status: statusQuiz);
                              }
                            });
                          },
                        ),
                                ...
                                await Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => MultiBlocProvider(providers: [
                                      BlocProvider(
                                        create: (context) => ViewAssigmentStudentCubit(
                                          assigmentModel: element,
                                        ),
                                      ),
                                    ], child: const ViewAssigmentStudentPage()),
                                  ),
                                );
                                // if (response != null) {
                                //   setState(() {
