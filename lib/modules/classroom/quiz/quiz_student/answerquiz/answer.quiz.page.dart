...
class AnserQuizPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AnserQuizCubit, AnserQuizCubitState>(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        ListTile(
                          ...
                          leading: Radio<int>(
                            value: 1,
                            groupValue: state.listAnser[i],
                            onChanged: (int? value) {
                              if (state.quizModel.quizAnserModel == null) context.read<AnserQuizCubit>().updateAnser(i, value);
                            },
                          ),
                        ),
                        ListTile(
                  ...
                  (state.quizModel.quizAnserModel == null)
                      ? Center(
                          child: ButtonNotClickMulti(
                              onTap: () {
                                bool check = true;
                                for (var i = 0; i < state.listAnser.length; i++) {
                                  if (state.listAnser[i] == null) {
                                    showToast(
                                      context: context,
                                ...
                                if (check) {
                                  context.read<AnserQuizCubit>().sendAnser();
                                }
                              },
                              child: Container(
                                width: 100,
