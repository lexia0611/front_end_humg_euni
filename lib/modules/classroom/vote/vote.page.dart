...
class VotePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<VoteCubit, VoteCubitState>(
      // buildWhen: (previous, current) => previous.voteModel!=current.voteModel|| previous.listVoteOptionModel.length!=current.listVoteOptionModel.length||  previous.selected!=current.selected,
      ...
      builder: (context, state) {
        ...
        return Scaffold(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        for (var index = 0; index < state.listVoteOptionModel.length; index++)
                          Row(
                            children: [
                              Expanded(
                                flex: 1,
                                child: Text("${state.listVoteOptionModel[index].count ?? 0} /${state.countStudent} "),
                              ),
                              Expanded(
                                flex: 7,
                                child: ListTile(
                                  title: Text(state.listVoteOptionModel[index].title ?? ""),
                                  leading: Radio<int>(
                                    value: index,
                                    groupValue: state.selected,
                                    onChanged: (int? value) {
                                      context.read<VoteCubit>().updateSelect(index, value);
                                    },
                                  ),
                                ),
                              ),
                            ...
                            onTap: () async {
                              var response = await showDialog(
                                  ...
                                  builder: (BuildContext context) {
                                    return CreateVoteOption(voteModel: state.voteModel!);
                                  });
                        ...
                        Center(
                          child: ButtonNotClickMulti(
                              onTap: () async {
                                var status = 0;
                                var data = state.voteModel!.copyWith(status: status);
          ...
          floatingActionButton: (state.voteModel == null)
              ? Container(
                  width: 50,
                  height: 50,
